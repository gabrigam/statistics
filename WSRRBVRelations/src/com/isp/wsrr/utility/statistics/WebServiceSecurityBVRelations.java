...
public class WebServiceSecurityBVRelations {
	...
	public static void main(String[] args) throws Exception {
		...
		String url = args[0];       //URL WSRR classico
		String legamibv = args[1];  //nome del file creato
		String path = args[2];      //path del file creato
		String user = args[3];
		...
		nbplog.info("----------------------LegamiBV per la Sicurezza V2.2 Settembre 2017 ---------------------------");
		nbplog.info("Fix endpoint multipli nello stesso ambiente con tipologia di interfaccia defferenti");
		nbplog.info("Fix enpoint incongruenti con specializzazzione");
		nbplog.info("Fix varie endpoint proxy");
		nbplog.info("Fix stampa in output del codice abilitativo");
		nbplog.info("----------------------------------------------------------------------------------------------");
									...
									if (ssa != null) {
										...
										if (acronimoCons != null) {
											...
											jsaedett = wsrrutility.getObjectPropertiesData(res,
													"&p1=name&p2=primaryType&p3=description&p4=gep63_ABILITAZ_INFRASTR", url, user, password);
											
											String consumer = WSRRUtility.getObjectValueFromJSONArrayData(   //Nicolosi Alberto - Aggiunta per stampa consumer in caso di error multiple endpoint
											...
											jsaedett = wsrrutility.getObjectPropertiesData(provider,
													"&p1=name&p2=primaryType&p3=description&p4=gep63_ABILITAZ_INFRASTR", url, user, password);  //Nicolosi Alberto - Aggiunta p4=gep63_ABILITAZ_INFRASTR
											
											String provid = WSRRUtility.getObjectValueFromJSONArrayData( //Nicolosi Alberto - Aggiunta per stampa provider in caso di error multiple endpoint
													(JSONArray) jsaedett.get(0), "name");
											...
											if (totalSOAPSec + totalRESTSec + totalCALLABLESec > 1)  //Nicolosi Alberto - Cambio logica codice per stmpare url di backend se presente o altrimenti url di frontend
												write = false;
											
											if ((totalSOAPSec !=0) || (totalRESTSec !=0) || (totalCALLABLESec != 0)){
												
												if (totalSOAPSec !=0) 
													currentEnpointName = SOAPEndpointSec[0];
												if (totalRESTSec !=0) 
													currentEnpointName = RESTEndpointSec[0];
												if (totalCALLABLESec !=0) 
													currentEnpointName = CALLABLEEndpointSec[0];		
												
											} else{
												
												currentEnpointName=GENERALEndpoint[0];
											...
											if (write) {
												...
												write=false;
												nbplog.error( "Multi endpoint found for SLA : " + bsrURISLA + " associated to SLD : " + sld  + " --- Consumer = " + consumer + " --- Provider = " + provid);
                                            }
											...
											if (write) {
												...
												System.out.println("Record : "+(contatore) +" len : "+i);  //Nicolosi Alberto - Modifica lunghezza l con numero totale legami processati
												contatore++;
